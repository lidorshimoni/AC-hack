...
void row::Draw()
{
	...
	if (type == M_FUNCTION)
	{
		...
		if (status != nullptr)
		{
			if (*status)
			{
				pmenu->font.Print(pos.x, pos.y, MENU_FONT_ON_COLOR, "%s%s", text, "ON");
			}
			else
			{
				pmenu->font.Print(pos.x, pos.y, MENU_FONT_OFF_COLOR, "%s%s", text, "OFF");
			}
		}
		else pmenu->font.Print(pos.x, pos.y, MENU_FONT_OFF_COLOR, "%s", text, text2);
	}
	else
	{
		vec3 pos = pmenu->font.centerText(pmenu->x, pmenu->nextRowY, pmenu->width, height, strlen(text) * MENU_FONT_WIDTH, MENU_FONT_HEIGHT);
		pmenu->font.Print(pos.x, pos.y, MENU_FONT_LABEL_COLOR, "%s", text);
	}
}
